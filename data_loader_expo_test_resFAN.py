...
class ExpoDataLoader():
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False, num_classes=10):
        ...
        self.data_files=[]
        fnames=os.listdir(os.path.join(gt_path))
        for fname in fnames:
    ...
    def preload_data(self):
        ...
        for fname in self.data_files:
            img, den, gt_count ,roi= self.read_image_and_gt_and_roi(fname)
    ...
    def read_image_and_gt_and_roi(self,fname):
        copy_of_fname = fname
        split_results_of_fname = copy_of_fname.split('_')
        possible_folder_name = split_results_of_fname[0]
        folder_name = possible_folder_name.split('-')[0]
        ...
        ###read roi
        roi_path=os.path.join(self.gt_path,'roi.csv')
