...
class ExpoDataLoader():
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False, num_classes=10):
        ...
        self.data_files=[]
        all_folders=os.listdir(gt_path)
        for single_folder in all_folders:
            fnames=os.listdir(os.path.join(gt_path,single_folder))
            for fname in fnames:
                if not operator.eq(fname, 'roi.csv'):
                    self.data_files.append(os.path.join(os.path.splitext(fname)[0] + '.jpg'))
    ...
    def __iter__(self):
        ...
        for idx in id_list:
            if self.pre_load:
                ...
                fname = files[idx]
                img, den, gt_count,roi = self.read_image_and_gt(fname)
                gt_class_label = np.zeros(self.num_classes,dtype=np.int)
                bin_val = (self.max_gt_count - self.min_gt_count)/float(self.num_classes)
    ...
    def get_stats_in_dataset(self):
        ...
        for fname in self.data_files:
            ...
            if operator.eq(fname,'100156_A02IndiaWE-03-S20100626080000000E20100626233000000_new.split.105_1'):
                stink=1
                stink=stink+1
                print(stink)
            den = pd.read_csv(os.path.join(self.gt_path,folder_name,os.path.splitext(fname)[0] + '.csv'), sep=',',header=None).as_matrix()
    ...
    def read_image_and_gt(self,fname):
        ...
        roi_path = os.path.join(self.gt_path,folder_name, 'roi.csv')
        roi = pd.read_csv(roi_path, sep=',', header=None).as_matrix()
