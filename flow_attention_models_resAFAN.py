...
class resFAN(nn.Module):
    ...
    def __init__(self, bn=False):
        ...
        self.appearance_B = nn.Conv2d(in_channels=in_dim, out_channels=in_dim//8, kernel_size=1)
        ...
        self.flow_A = nn.Conv2d(in_channels=in_dim, out_channels=in_dim//8, kernel_size=1)
        self.flow_B = nn.Conv2d(in_channels=in_dim, out_channels=in_dim//8, kernel_size=1)
        self.flow_C = nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=1)
        ...
        for param in self.own_reslayer_3.parameters():
            param.requires_grad=False
        self.backend.load_state_dict(pre_wts2,strict=False)
        self.convDU.load_state_dict(pre_wts2, strict=False)
        self.convLR.load_state_dict(pre_wts2, strict=False)

        #flow
        self.frontend_flow = nn.Sequential(
            res.conv1, res.bn1, res.relu, res.maxpool, res.layer1, res.layer2
        )
        for param in self.frontend_flow.parameters():
            param.requires_grad=False
        self.own_reslayer_3_flow = make_res_layer(Bottleneck, 256, 23, stride=1)
        for param in self.own_reslayer_3_flow.parameters():
            param.requires_grad=False
        self.own_reslayer_3_flow.load_state_dict(res.layer3.state_dict())
        self.backend_flow.load_state_dict(pre_wts2,strict=False)
        self.convDU_flow.load_state_dict(pre_wts2, strict=False)
        self.convLR_flow.load_state_dict(pre_wts2, strict=False)
    ...
    def forward(self, in_data):
        ...
        proj_key_x = self.flow_B(f).view(m_batchsize, -1, width*height)
        ...
        #proj_key_f = self.appearance_B(x).view(m_batchsize_f, -1, width_f*height_f)
        #energy_f = torch.bmm(proj_query_f, proj_key_f)
        #attention_f = self.softmax(energy_f)
